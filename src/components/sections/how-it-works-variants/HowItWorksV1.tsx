...
const STEPS = [
  {
    number: "01",
    title: "경험을 기록해요",
    description: "오늘 있었던 일을 짧게 메모하듯 기록해요. 거창하지 않아도 괜찮아요.",
...
export default function HowItWorksV1() {
  ...
  const step1 = useScrollReveal(0.15);
  const step2 = useScrollReveal(0.15);
  const step3 = useScrollReveal(0.15);
  const stepReveals = [step1, step2, step3];
              ...
              <Fragment key={i}>
                ...
                {i < STEPS.length - 1 && (
                  <div
                    ...
                    style={{
                      opacity: reveal.visible ? 1 : 0,
                      transition: `opacity 0.5s ease ${i * 120 + 400}ms`,
                    }}
                  >
                    <ArrowIcon />
                  </div>
