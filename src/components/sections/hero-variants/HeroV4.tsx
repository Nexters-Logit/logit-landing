...
const FLOAT_CARDS = [
  { text: "편의점 알바", tag: "갈등 관리", color: "bg-icon-1", delay: "0s", x: "10%", y: "5%" },
  { text: "동아리 총무", tag: "조직 관리", color: "bg-icon-4", delay: "0.4s", x: "55%", y: "12%" },
  { text: "여행 계획짜기", tag: "기획력", color: "bg-icon-7", delay: "0.8s", x: "25%", y: "55%" },
  { text: "팀플 리더", tag: "리더십", color: "bg-icon-3", delay: "1.2s", x: "65%", y: "60%" },
  { text: "매일 가계부", tag: "데이터 분석", color: "bg-icon-5", delay: "1.6s", x: "40%", y: "30%" },
];
      ...
      <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
        <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left gap-8">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold text-grey-400 leading-[1.2] [word-break:keep-all]">
            ...
            자소서 초안은 AI가<br />
            <span className="text-primary-200">10초 만에 끝냅니다.</span>
          </h1>
        ...
        <div className="flex-1 w-full">
          <div className="relative w-full aspect-square">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-32 h-32 lg:w-48 lg:h-48 rounded-full bg-primary-50 flex items-center justify-center shadow-lg">
                <Image src="/logo_symbol_2d.svg" alt="Logit" width={64} height={64} />
              </div>
            </div>
            ...
            {FLOAT_CARDS.map((card) => (
              <div
                key={card.tag}
                className="float-card absolute bg-white border border-grey-70 rounded-2xl px-4 py-3 shadow-md flex flex-col gap-1.5 w-36 lg:w-44"
                style={{ left: card.x, top: card.y, animationDelay: card.delay }}
              >
                <p className="text-grey-400 text-sm font-medium">{card.text}</p>
                <span className={`${card.color} text-white text-xs font-semibold px-2.5 py-1 rounded-full w-fit`}>
                  {card.tag}
                </span>
              </div>
            ))}
